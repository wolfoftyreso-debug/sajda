...
import { assertNamePackageQuery, readNamePackageBody, readNamePackageHeader, sendNamePackageJson, setNamePackageResponseHeaders, type NamePackageHttpRequest, type NamePackageHttpResponse } from "../_shared/name-package-http.js";
import { createRequestId } from "../_shared/public-api.js";

export const config = { maxDuration: 30 };

/** Scoped server-to-server identity exploration, using the shared MCP executor. */
export function createNamePackagesApiHandler(dependencies: {
  ...
  return async (request: NamePackageHttpRequest, response: NamePackageHttpResponse): Promise<void> => {
    ...
    try {
      ...
      const result = await (dependencies.execute ?? executeMcpProduct)("name_packages_search", input, principal);
...
export default createNamePackagesApiHandler();
