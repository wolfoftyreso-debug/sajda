...
For reviewed testing only, migration `0017_ai_test_allowance.sql` permits the server-only `SAJDA_AI_TEST_IP_DAILY_LIMIT` override (integer 1–20) in Preview/Development. Without it the limit remains three; Production ignores valid test overrides and still allows only three. Invalid configured values fail closed. The override uses the existing environment namespace, HMAC identity and consumed UTC-day counters: changing or removing it never resets spent requests. Global 50/default, 100/hard maximum, two 20-second leases, provider deadlines and any Vercel monetary spend cap remain unchanged. Apply the additive migration to the reviewed nonproduction database before enabling the override, and verify target/counters before and after; do not rewrite migration 0003 or clear allowance rows. The public interface must describe any denied AI attempt as local-rule fallback, not successful AI generation.
