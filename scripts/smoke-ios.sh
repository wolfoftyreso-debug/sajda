...
finish_smoke() {
  ...
  trap - EXIT
  if [ "$result" -ne 0 ]; then
    # Observe before deleting the simulator, even if simctl launch timed out.
    # A diagnostic failure must neither suppress cleanup nor turn failure green.
    run_step failure-screenshot 30 xcrun simctl io "$SAJDA_SIMULATOR_ID" screenshot ios/App/build/sajda-iphone-smoke.png || true
    run_step failure-evidence 45 node scripts/collect-ios-diagnostics.mjs "$SAJDA_SIMULATOR_ID" ios/App/build || true
  fi
  run_step shutdown 30 xcrun simctl shutdown "$SAJDA_SIMULATOR_ID" || true
  run_step delete 30 xcrun simctl delete "$SAJDA_SIMULATOR_ID" || true
  exit "$result"
}
...
# Cold hosted devices can finish system migration but still have LaunchServices
# busy. Bound the command independently; the subsequent UI gate is unchanged.
run_step launch 120 env SIMCTL_CHILD_SAJDA_CI_SMOKE=1 xcrun simctl launch "$SAJDA_SIMULATOR_ID" com.hypbit.sajda
