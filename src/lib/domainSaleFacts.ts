...
const copy: Record<Language, {
  ...
  sv: {
    ...
    note: [
      "En historisk uppgift, inte ett aktuellt begärt pris eller en värdering av en annan domän.",
      "Användbar bakgrund, men ingen genväg till att prissätta ett annat namn.",
    ],
  ...
  fr: {
    ...
    note: [
      "Une donnée historique, pas un prix demandé actuel ni une estimation d’un autre domaine.",
...
const marketPulseCopy: Record<Language, {
  ...
  sv: {
    category: "Marknadsöversikt",
    badge: "AGGREGERAD DATA",
    title: (tld, average) => `${tld} · genomsnittligt rapporterat försäljningspris ${average}`,
    body: (source, period, count, highest, checkedAt) => `Sammanställning från ${source} för ${period}: ${count} rapporterade försäljningar; högsta rapporterade pris ${highest}. Uppgifter hämtade ${checkedAt}. Historiska sammanställda data, inte en värdering, offert, tillgänglighetskontroll eller enskild försäljning.`,
    source: (source) => `Läs ${source}`,
  },
  es: {
    category: "Panorama del mercado",
    badge: "DATOS AGREGADOS",
    title: (tld, average) => `${tld} · precio medio de venta publicado: ${average}`,
    body: (source, period, count, highest, checkedAt) => `Resumen de ${source} para ${period}: ${count} ventas publicadas; precio más alto publicado ${highest}. Datos consultados el ${checkedAt}. Solo datos históricos agregados: no son una valoración, una oferta, un resultado de disponibilidad ni una venta individual.`,
    source: (source) => `Leer ${source}`,
  },
  fr: {
    category: "Aperçu du marché",
    badge: "DONNÉES AGRÉGÉES",
    title: (tld, average) => `${tld} · prix de vente moyen publié : ${average}`,
    body: (source, period, count, highest, checkedAt) => `Synthèse de ${source} sur ${period} : ${count} ventes publiées ; prix publié le plus élevé ${highest}. Données relevées le ${checkedAt}. Données historiques agrégées uniquement : ni estimation, ni offre, ni résultat de disponibilité, ni vente individuelle.`,
    source: (source) => `Lire ${source}`,
  },
  zh: {
    category: "市场概览",
    badge: "汇总数据",
    title: (tld, average) => `${tld} · 已报道交易的平均价格 ${average}`,
