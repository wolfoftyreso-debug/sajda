...
export default function NativeNavigation() {
  ...
  const items = [
    ...
    { to: "/watchlist", label: copy.saved, icon: Heart },
  ...
  return <nav aria-label={copy.navigation} className="sajda-native-navigation fixed inset-x-0 bottom-0 z-40 border-t border-border bg-card/95 backdrop-blur-xl">
    <div className="mx-auto grid h-16 max-w-xl grid-cols-5 items-stretch px-1">
      {items.map(({ to, label, icon: Icon }) => <NavLink key={to} to={to} end aria-label={label}
        ...
        <Icon className="h-5 w-5 shrink-0" aria-hidden="true" /><span className="max-w-full truncate">{label}</span>
      </NavLink>)}
    </div>
  </nav>;
}
