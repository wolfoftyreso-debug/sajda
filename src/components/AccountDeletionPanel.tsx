...
export default function AccountDeletionPanel({ accountId, onDeleted }: { accountId: string; onDeleted: (accountId: string) => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    setOpen(false); setAcknowledged(false); setChallenge(null); setCode(""); setBusy(null); setError(""); setDeleted(false);
  ...
  async function perform(action: "request" | "confirm") {
    ...
    try {
      ...
      setError(failureCode === "deletion_code_invalid" ? copy.invalid
        : failureCode === "deletion_rate_limited" ? copy.limited
        : failureCode === "deletion_email_unavailable" ? copy.emailError : action === "request" ? copy.requestError : copy.uncertain);
    } finally {
  ...
  function close() {
    ...
    setOpen(false); setAcknowledged(false); setChallenge(null); setCode(""); setError(""); requestId.current = null;
  }
      ...
      : <>
        ...
        {!open ? <Button variant="outline" className="mt-4 min-h-11 whitespace-normal text-destructive" onClick={() => setOpen(true)} aria-expanded={false}>{copy.title}</Button>
          : <div className="mt-4 space-y-4">
            ...
            {error && <p id={`${id}-error`} role="alert" className="text-sm leading-relaxed text-destructive">{error}</p>}
            <div className="flex flex-wrap gap-3">
              ...
              {challenge && <Button variant="ghost" disabled={busy !== null} className="min-h-11 h-auto whitespace-normal" onClick={() => {
                setChallenge(null); setCode(""); setError(""); requestId.current = null;
              }}>{copy.resend}</Button>}
            </div>
          </div>}
      </>}
