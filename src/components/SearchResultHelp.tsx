...
const copy: Record<Language, ResultHelpCopy> = {
  sv: {
    ...
    sections: [
      { title: "Är domänen ledig?", body: "Ledig är ett registerbesked vid söktillfället, inte en bokning. Okänd betyder att kontrollen inte kunde ge ett säkert besked. Bekräfta tillgängligheten hos leverantören före köp." },
  ...
  en: {
    ...
    sections: [
      { title: "Is the domain available?", body: "Available is a registry result at the time of the search, not a reservation. Unknown means the check could not confirm its status. Confirm availability with the provider before buying." },
      { title: "What does the price mean?", body: "A published standard price applies to the extension, not an exact-domain quote. If a confirmed price is missing, no estimated amount is shown. Check the final price, renewal, tax and fees with the provider." },
  ...
  es: {
    ...
    sections: [
      { title: "¿Está disponible el dominio?", body: "Disponible es el resultado del registro en el momento de la búsqueda, no una reserva. Desconocido significa que la consulta no pudo confirmar el estado. Confirma la disponibilidad con el proveedor antes de comprar." },
  ...
  fr: {
    ...
    sections: [
      { title: "Le domaine est-il disponible ?", body: "Disponible correspond au résultat du registre au moment de la recherche, pas à une réservation. Inconnu signifie que la vérification n'a pas confirmé le statut. Confirmez la disponibilité auprès du fournisseur avant l'achat." },
  ...
  zh: {
    ...
    sections: [
      { title: "域名可以注册吗？", body: "可用仅表示搜索时的注册局查询结果，并非预订。未知表示本次查询无法确认状态。购买前请向服务商确认域名是否可注册。" },
