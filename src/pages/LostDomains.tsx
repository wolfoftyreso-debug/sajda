          ...
          {report?.latestRun ? <section id="trading-results" tabIndex={-1} aria-labelledby="plus-report-title" className={`${panel} scroll-mt-6 overflow-hidden outline-none`}>
            <header className="border-b border-border p-5 sm:p-6">
              ...
              <div className="mt-5 grid min-w-0 gap-3 sm:grid-cols-2 xl:grid-cols-4">
                <label className="min-w-0 text-xs font-medium">{tradingText(copy.locale, "Filter domain")}<input value={query} onChange={event=>setQuery(event.target.value)} maxLength={253} placeholder={tradingText(copy.locale, "Search this report…")} className="mt-1 block min-h-11 w-full min-w-0 rounded-lg border border-input bg-background px-3 text-base font-normal focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" /></label>
                <label className="min-w-0 text-xs font-medium">{tradingText(copy.locale, "Status")}<select value={filter} onChange={event=>setFilter(event.target.value as TradingReportFilter)} className="mt-1 block min-h-11 w-full min-w-0 rounded-lg border border-input bg-background px-3 text-base font-normal focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                ...
                <label className="min-w-0 text-xs font-medium">{tradingText(copy.locale, "Extension")}<select value={tld} onChange={event=>setTld(event.target.value)} className="mt-1 block min-h-11 w-full min-w-0 rounded-lg border border-input bg-background px-3 text-base font-normal focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><option value="">{tradingText(copy.locale, "All extensions")}</option>{tlds.map(value=><option key={value} value={value}>.{value}</option>)}</select></label>
